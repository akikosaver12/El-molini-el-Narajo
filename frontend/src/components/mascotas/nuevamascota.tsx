...
const GOLD_STYLES = `
  ...
  .edad-badge { display:flex; align-items:center; gap:8px; padding:8px 14px; border-radius:0.75rem; background:rgba(201,168,76,0.1); border:1px solid rgba(201,168,76,0.25); }
`;
...
const CrearMascota: React.FC = () => {
  ...
  const removeImage = () => setFormData((prev) => ({ ...prev, imagen: null }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { alert("Trabajador creado con éxito"); navigate("/mascotas"); }
      else alert("Error: " + (json.msg || json.error || JSON.stringify(json)));
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 border border-amber-700/50 rounded-full px-5 py-2 mb-6 bg-amber-950/30">
            ...
            <span className="text-xs tracking-[0.3em] uppercase" style={{ color: '#C9A84C' }}>Nuevo Registro</span>
          </div>
          <h1 className="text-4xl font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif", color: '#F5F0E8' }}>
            Registrar <span className="gold-text">Trabajador</span>
          </h1>
          <p className="font-light" style={{ color: '#D4C5A0' }}>
            Completa la información para agregar el nuevo trabajador
          </p>
          <GoldDivider />
        </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="relative">
            <div
              className={`w-48 h-48 photo-zone ${dragActive ? 'drag-active' : ''} ${formData.imagen ? 'has-image' : ''}`}
            ...
            <label htmlFor="imagen" className="absolute bottom-4 right-4 photo-upload-btn">
              {formData.imagen ? <Upload size={20} /> : <Camera size={20} />}
            </label>
            {formData.imagen && (
            ...
            <input id="imagen" type="file" name="imagen" accept="image/*" onChange={handleFileChange} className="hidden" />
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="rounded-2xl p-8 mb-8" style={{ background: 'rgba(59,130,246,0.04)', border: '1px solid rgba(59,130,246,0.12)' }}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" name="nombre" value={formData.nombre} onChange={handleChange} required className="gold-input" placeholder="Ej: Juan Pérez García" />
                </div>
                <div>
                  <FieldLabel>Tipo de Documento *</FieldLabel>
                  <select name="tipoDocumento" value={formData.tipoDocumento} onChange={handleChange} required className="gold-select">
                ...
                <div>
                  ...
                  <input type="text" name="numeroDocumento" value={formData.numeroDocumento} onChange={handleChange} required className="gold-input" placeholder="Ej: 1234567890" />
                </div>
                <div>
                  <FieldLabel>Estado Civil *</FieldLabel>
                  <select name="estadoCivil" value={formData.estadoCivil} onChange={handleChange} required className="gold-select">
                ...
                <div>
                  ...
                  <input type="date" name="fechaNacimiento" value={formData.fechaNacimiento} onChange={handleChange} required max={new Date().toISOString().split('T')[0]} className="gold-input" />
                  {formData.fechaNacimiento && (
                    <div className="edad-badge mt-3">
                ...
                <div>
                  ...
                  <select name="genero" value={formData.genero} onChange={handleChange} required className="gold-select">
            ...
            <div className="rounded-2xl p-8 mb-8" style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.12)' }}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="date" name="fechaExamenOcupacional" value={formData.fechaExamenOcupacional} onChange={handleChange} required className="gold-input" />
                </div>
                <div>
                  <FieldLabel>Tallaje de Botas *</FieldLabel>
                  <select name="tallajeBotas" value={formData.tallajeBotas} onChange={handleChange} required className="gold-select">
                ...
                <div>
                  ...
                  <select name="especie" value={formData.especie} onChange={handleChange} required className="gold-select">
                ...
                <div>
                  ...
                  <input type="text" name="raza" value={formData.raza} onChange={handleChange} required className="gold-input" placeholder="Ej: 7:00 AM - 5:00 PM" />
                </div>
                <div className="md:col-span-2">
                  <FieldLabel>Estado del Trabajador *</FieldLabel>
                  <select name="estado" value={formData.estado} onChange={handleChange} required className="gold-select">
                    <option value="">Seleccionar estado</option>
            ...
            <div className="flex justify-center">
              <button
                ...
                className={`gold-btn py-5 px-16 rounded-2xl flex items-center gap-4 text-lg tracking-widest uppercase ${isSubmitting ? 'animate-pulse' : ''}`}
              >
                {isSubmitting ? (
                  <>
                  ...
                  <><Check size={24} /> Registrar Trabajador</>
                )}
              </button>
            </div>
          </form>
        ...
        <div className="text-center mt-8">
          <p className="text-base mb-2" style={{ color: '#8B6914' }}>Los campos marcados con * son obligatorios</p>
          <p className="text-sm" style={{ color: '#5a4a20' }}>Formatos de imagen soportados: JPG, PNG, WebP (máx. 5MB)</p>
        </div>
      </div>
    </div>
  );
